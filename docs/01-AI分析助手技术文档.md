...
[agent/intents.py] → detect(question)
    ├── LLM Function Calling: DeepSeek API (temperature=0, 5s timeout)
    │   └── [agent/planner.py] → create_plan() 结构化分析计划
    └── 规则引擎回退: 关键词匹配 (无网络依赖, 毫秒级)
    ↓
[agent/tools.py] → 8个参数化SQL工具 (100% %s占位符)
    └── [agent/catalog.py] → DatasetCatalog 表/列白名单校验
    ↓
[agent/responder.py] → make_response(intent_info, tool_result)
    ├── [agent/statistics.py] → 线性回归 / Z-score异常 / 滑动平均
    ├── [agent/alerts_engine.py] → 5条预置预警规则评估
    └── [agent/chart_spec.py] → ChartSpec 图表映射
    ↓
[agent/schemas.py] → AnalysisPlan 结构化分析合约
    ↓
返回 JSON: {answer, intent, table?, chart?, statistics?, alerts?}
    ↓
