<template>
  <nav class="right-nav">
    <button v-for="item in navItems" :key="item.id"
      :class="['rn-item', { active: activePage === item.id }]"
      @click="$emit('select', item.id)" :title="item.label">
...
.rn-item::before {
  ...
  background: var(--ci-primary); opacity: 0; transition: opacity .3s var(--ci-ease-out);
}
.rn-item:hover { color: var(--ci-primary); }
.rn-item:hover .rn-icon { transform: scale(1.12); }
.rn-item.active { background: var(--ci-primary); color: #fff; animation: pulse-glow 3s ease-in-out infinite; }
.rn-item.active .rn-icon { transform: scale(1.05); }
.rn-icon { font-size: 18px; line-height: 1; transition: transform .3s var(--ci-ease-spring); position: relative; z-index: 1; }
.rn-label { font-size: 12px; font-weight: 600; line-height: 1; position: relative; z-index: 1; }
...
.rn-ai-btn:active { transform: scale(0.94); }
@media (max-width: 900px) { .right-nav { display: none; } }
</style>
