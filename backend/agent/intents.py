...
def detect_rules(question: str) -> dict:
    ...
    if any(w in q for w in ["你是谁", "你叫什么", "你的名字", "介绍一下自己", "你是干什么的"]):
        return {"intent": "chat", "year": year}
    if any(w in q for w in ["你好", "嗨", "hello", "hi"]) and len(q) <= 5:
        return {"intent": "chat", "year": year}

    # ── KPI / 全球均温 ──
    if any(w in q for w in ["全球平均", "年均温", "平均气温", "kpi", "指标", "全球气候"]):
        return {"intent": "kpi", "year": year}

    # ── 月度 (含月份范围) ──
    if any(w in q for w in ["月度", "每月", "各月", "12个月", "月均", "月气温", "月温度"]):
        result = {"intent": "monthly", "year": year}
        mo_match = _extract_months(question)
        if mo_match:
            result["months"] = mo_match
        return result

    # ── 排名 — 最热 ──
    if any(w in q for w in ["最热", "最高温", "高温", "最烫", "温度最高"]):
    ...
    # ── 排名 — 降水 ──
    if any(w in q for w in ["降水", "下雨", "最多雨", "降雨", "雨水", "雨量", "潮湿"]):
        limit = _extract_limit(q)
        return {"intent": "ranking", "category": "rainiest", "limit": limit, "year": year}

    # ── 排名 — 极端 ──
    if any(w in q for w in ["极端", "恶劣天气", "极端天气", "糟糕天气"]):
        limit = _extract_limit(q)
...
def _extract_months(question: str) -> list | None:
    """从问题中提取月份范围，如 '5到10月' → [5,6,7,8,9,10]"""
